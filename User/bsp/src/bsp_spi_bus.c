...
#if USE_SPI1_EN == 1
  if (_spi == SPI1)
    HAL_SPI_TransmitReceive(&hspi1, (uint8_t *)spi1_TxBuf, (uint8_t *)spi1_RxBuf, spi1_len, 100);
#endif

#if USE_SPI2_EN == 1
  if (_spi == SPI2)
    HAL_SPI_TransmitReceive(&hspi2, (uint8_t *)spi2_TxBuf, (uint8_t *)spi2_RxBuf, spi2_len, 100);
#endif

#if USE_SPI3_EN == 1
  if (_spi == SPI3)
    HAL_SPI_TransmitReceive(&hspi3, (uint8_t *)spi3_TxBuf, (uint8_t *)spi3_RxBuf, spi3_len, 100);
#endif

#if USE_SPI4_EN == 1
  if (_spi == SPI4)
    HAL_SPI_TransmitReceive(&hspi4, (uint8_t *)spi4_TxBuf, (uint8_t *)spi4_RxBuf, spi4_len, 100);
#endif

#if USE_SPI5_EN == 1
  if (_spi == SPI5)
    HAL_SPI_TransmitReceive(&hspi5, (uint8_t *)spi5_TxBuf, (uint8_t *)spi5_RxBuf, spi5_len, 100);
#endif

#if USE_SPI6_EN == 1
  if (_spi == SPI6)
    HAL_SPI_TransmitReceive(&hspi6, (uint8_t *)spi6_TxBuf, (uint8_t *)spi6_RxBuf, spi6_len, 100);
#endif
#endif

--以下快捷方式将显示在PC软件界面(STM8L)
--F01=自动编程,start_prog()
--F03=擦除flash,erase_chip(0x08000)
--F04=擦除eeprom,erase_chip(0x01000)
--F05=打印flash,print_flash(0x08000,1024,32,0x08000)
--F06=打印eeprom,print_flash(0x01000,256,32,0x01000)
...
function cofig_chip1(void)	
	...
	FLASH_SIZE = 32 * 1024		--定义FLASH总容量
	EEPROM_ADDRESS = 0x001000 	--定义FLASH起始地址(STM8S和STM8L不同）
	EEPROM_SIZE = 1 * 1024  	--定义EEPROM容量
	UID_ADDR = 0x4926			--UID地址，不同的CPU不同   		

	--任务列表，可以任意追加
	--数据文件和lua文件同目录.支持../上级目录,也可以写绝对路径
	TaskList = {
		"0:/H7-TOOL/Programmer/User/TestBin/32K_55.bin",--数据文件 (""表示忽略)
		0x008000,										--目标地址 (0x008000 Flash)
		
		"0:/H7-TOOL/Programmer/User/TestBin/1K.bin",	--数据文件 (""表示忽略)
		0x001000,										--目标地址 (0x001000 EEPROM)	
	}		
	...
	OB_RDP_OFF     = "AA 00 00 00 00 00 00 00"	--OPTION缺省值,用于解除保护
	OB_RDP_ON      = "00 00 00 00 00 00 00 00"	--读保护的值，用于解除保护		
